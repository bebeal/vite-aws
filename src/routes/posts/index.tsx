          ...
          {posts.map((post: Post) => (
            <li key={post.id} className="border-b border-gray-400 dark:border-gray-600 last:border-b-0">
              <Link
                ...
                className="flex justify-between items-center gap-8 p-4 min-w-[500px] dark:text-white text-black"
              >
                <span>{post.title}</span>
                {post.date && (
                  <span className="text-sm dark:text-gray-200 whitespace-nowrap">
                    {post.date}
                  </span>
                )}
              </Link>
